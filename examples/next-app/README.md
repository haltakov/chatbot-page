...
lib/chatbot-base-config.ts    Persona (name, title, handle) and intro message
...
The flow: `page.tsx` loads the canned answers and first-launch content on the server and passes them to `chatbot.tsx`, which assembles the `ChatApp` config. Canned answers are tried first; anything unmatched is sent to `/api/chat`, which streams from OpenAI when a key is set and from a placeholder otherwise.
